...
- 使用对象：**所有人**
- 功能：下载当前课程的指定文件（资料或作业）到指定文件路径（Hint：本质复制/写入或追加写入），并输出文件内容，执行操作顺序是先处理文件保存，再处理重定向（Hint：意思是如果同时使用，则会先保存文件，再重定向输出到指定文件，假如路径地址相同，则此时是 `>` 则会覆盖原文件，相当于没有重定向操作，直接保存了文件，如果是 `>>` 则会追加到原文件末尾，相当于原文件在一个文件中重复了两遍）
- 前置条件：已登录、已选择课程
- 备注：**评测会保证仅操作文本文件**
