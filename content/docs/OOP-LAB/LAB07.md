...
- 方法 `public IIterator iterator() {return new SequenceIterator();}`
- 具有内部类 `private class SequenceIterator implements IIterator`，它用于序列遍历的迭代器
  - 自定义必要的属性。
...
- 在实现本题的 `toString` 时，推荐使用 `StringBuilder`或`StringJoiner`。
